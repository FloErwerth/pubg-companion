...
import {
	gapBetweenItems,
	styles,
} from "~/components/SelectableButtonGroup/styles";
		...
		() =>
			options.map((option, index) => {
				...
				const numberOfItemsInRow =
					r < fullRows ? maxItemsInRow : options.length - r * maxItemsInRow;
				const padding = pagePadding * 2;
				const accumulatedGap = numberOfItemsInRow * gapBetweenItems;
				const itemWidth =
					(width - padding - accumulatedGap) / numberOfItemsInRow;
